...
```
╭─ DB scoping audit — gq_direct_payments_backend ─────────────────────╮
...
Inventory
  ...
  Ambient surface: imports 521 tables (master_models.py), queries 24 → 497 unused
  Δ vs Run #0: initial audit

By owning domain
  🔴 2  🟠 1        identity   institutes(R), student_profile(RW)   3 open
  🟡 1             lending    lender_config(R)                     1 open

Top 5 by dependency risk
  🔴 dbdep-identity-001   student_profile   webhook_helper.py:210   foreign-write + cross-domain-txn
...
- "Audit the DB scope of gq_direct_payments_backend"
...
Ambient: imports master_models.py (521 tables), queries 24 → 497 unused

1. Needs: read access to identity-service (stop touching institutes, student_profile)
   - CRITICAL: webhook_helper.py:210 UPDATEs student_profile inside the same
     atomic() as the payment write — two writers + shared transactional fate
   - HIGH: dt_payment_order JOIN institutes couples both schemas
...
2. Needs: reference access to lending-service (lender_config)
   ...

Owned tables to keep:
   - dt_payment_order, dt_transaction, settlement_*, *_webhook, idempotency tables

Ambient surface to shed:
   - Narrow master_models import from 521 tables to owned + declared deps

Confirm ownership (inferred):
   - institutes → identity · lender_config → lending
```
